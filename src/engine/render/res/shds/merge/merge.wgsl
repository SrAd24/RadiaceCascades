...
fn fillTexture0(globalPos: vec2u) {
  ...
  for (var i: u32 = 0; i < u32(probeSize); i++) {
    for (var j: u32 = 0; j < u32(probeSize); j++) {
      color += vec3f(textureLoad(resultTexture, globalPos * u32(probeSize) + vec2u(j, i), 0).r, 0, 0);
      color += vec3f(0, textureLoad(resultTexture, globalPos * u32(probeSize) + vec2u(j, i), 1).r, 0);
      color += vec3f(0, 0, textureLoad(resultTexture, globalPos * u32(probeSize) + vec2u(j, i), 2).r);
    }
  }
...
fn fillFinalTexture(globalPos: vec2u) {
  var color: vec3f;
  if (textureLoad(distanceTexture, globalPos).r == 0) {  
    color = vec3f(textureLoad(baseColorTexture, globalPos, 0).r,
                  textureLoad(baseColorTexture, globalPos, 1).r,
                  textureLoad(baseColorTexture, globalPos, 2).r);
  } else {
    var cascadeMaxIndex: f32 = glbData.cascadeMaxIndex;
    var frameSize: f32 = glbData.frameSize;
