    ...
    if (parsedData.materials && parsedData.materials.length > 0) {
      for (const gltfMaterial of parsedData.materials) {
        ...
        const material = await this.render.createMaterial({
          ...
          roughness: metallicRoughnessTexture || roughness,
          metallic: metallicRoughnessTexture || metallic,
          emissive: emissiveTexture || new vec3(emission[0], emission[1], emission[2]),
