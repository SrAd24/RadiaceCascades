 ...
 protected get render(): render {
  ...
  public async create(materialParams: material_descriptor) {
    ...
    if (typeof(materialParams.roughness) === "string")
      await this.albedo.updateByImage(materialParams.roughness);
    if (typeof(materialParams.metallic) === "string")
      await this.albedo.updateByImage(materialParams.metallic);
    if (typeof(materialParams.normalMap) === "string")
      await this.albedo.updateByImage(materialParams.normalMap);
    if (materialParams.texturesDirName) {
      await this.roughness.updateByImage(materialParams.texturesDirName);
      await this.metallic.updateByImage(materialParams.texturesDirName);
      await this.normalMap.updateByImage(materialParams.texturesDirName);
    }
    else {
